...
case class ProfileBox(
  ...
  profilePictureRegister: StringRegister,
  followingRegister: CollStringRegister,
  override val tokens: Seq[ErgoToken] = Seq(new ErgoToken(dumdumdumsProfileToken, 1)),
  ...
  override def getContract(implicit ctx: BlockchainContext): ErgoContract =
    ProfileBoxContract.getContract(addressRegister.getAddress).contract.ergoContract
...
object ProfileBox {
  def get(address: Address)(implicit client: Client): InputBox = {
    ...
    val profileInputBox: InputBox = client.getCoveringBoxesFor(ProfileBoxContract.getContract(address).contract.ergoContract.toAddress,
  ...
  def from(inputBox: InputBox): ProfileBox = {
    ProfileBox(
      ...
      addressRegister = new AddressRegister(inputBox.getRegisters.get(0).getValue.asInstanceOf[Coll[Byte]].toArray),
      profilePictureRegister = new StringRegister(inputBox.getRegisters.get(1).getValue.asInstanceOf[Coll[Byte]].toArray),
      followingRegister = new CollStringRegister(inputBox.getRegisters.get(2).getValue.asInstanceOf[Coll[Coll[Byte]]].toArray),
      box = Option(Box(inputBox))
    )
  }

  def changeNFT(profileBox: ProfileBox, nftId: String): ProfileBox = {
    profileBox.copy(profilePictureRegister = new StringRegister(nftId))
  }
}

case class ProfileTokenDistributionBox(
  override val id: ErgoId,
...
object ProfileTokenDistributionBox extends BoxWrapperHelper {
  ...
  def decrementProfileToken(profileTokenDistributionBox: ProfileTokenDistributionBox): ProfileTokenDistributionBox ={
...
object NFTHelper {
  def getNFTInputBox(nftId: String)(explorer: Explorer, ctx: BlockchainContext): InputBox = {
